...
public class MyDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final int DATABASE_VERSION = 8; // Ajout Medication History
    ...
    // --- TABLE MEDICATION HISTORY ---
    private static final String TABLE_MED_HISTORY = "medication_history";
    public static final String COL_HIST_ID = "id";
    public static final String COL_HIST_MED_ID = "med_id";
    public static final String COL_HIST_MED_NAME = "med_name";
    public static final String COL_HIST_SCHEDULED_TIME = "scheduled_time";
    public static final String COL_HIST_TAKEN_TIME = "taken_time";
    public static final String COL_HIST_STATUS = "status"; // 'TAKEN', 'MISSED'
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TABLE_MED_HISTORY + " (" +
                COL_HIST_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                COL_HIST_MED_ID + " INTEGER, " +
                COL_HIST_MED_NAME + " TEXT, " +
                COL_HIST_SCHEDULED_TIME + " TEXT, " +
                COL_HIST_TAKEN_TIME + " TEXT, " +
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_MED_HISTORY);
        onCreate(db);
    }
    ...
    // ==========================================
    public void addUser(String fullname, String email, String password, int week, String symptoms, double weight,
    ...
    public boolean checkEmail(String email) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_EMAIL + " = ?",
                new String[] { email });
        boolean exists = cursor.getCount() > 0;
        cursor.close();
        return exists;
    ...
    public SleepSession getLastSleepSession() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(
                "SELECT * FROM " + TABLE_SLEEP_SESSIONS + " ORDER BY " + COL_SLEEP_ID + " DESC LIMIT 1", null);
    ...
    // ==========================================
    // PARTIE 5 : HISTORIQUE MÉDICAMENTS
    // ==========================================
    public void addMedicationHistory(int medId, String medName, String scheduledTime, String takenTime, String status,
            String date) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(COL_HIST_MED_ID, medId);
        cv.put(COL_HIST_MED_NAME, medName);
        cv.put(COL_HIST_SCHEDULED_TIME, scheduledTime);
        cv.put(COL_HIST_TAKEN_TIME, takenTime);
        cv.put(COL_HIST_STATUS, status);
        cv.put(COL_HIST_DATE, date);
        db.insert(TABLE_MED_HISTORY, null, cv);
    }

    public Cursor getHistoryByDate(String date) {
        SQLiteDatabase db = this.getReadableDatabase();
        return db.rawQuery("SELECT * FROM " + TABLE_MED_HISTORY + " WHERE " + COL_HIST_DATE + " = ?",
                new String[] { date });
    }

    public int getAdherencePercentage() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor taken = db.rawQuery(
                "SELECT COUNT(*) FROM " + TABLE_MED_HISTORY + " WHERE " + COL_HIST_STATUS + " = 'TAKEN'", null);
        Cursor total = db.rawQuery("SELECT COUNT(*) FROM " + TABLE_MED_HISTORY, null);

        int takenCount = 0;
        int totalCount = 0;

        if (taken.moveToFirst())
            takenCount = taken.getInt(0);
    ...
    public String getMostMissedMedication() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + COL_HIST_MED_NAME + ", COUNT(*) as count FROM " + TABLE_MED_HISTORY +
                " WHERE " + COL_HIST_STATUS + " = 'MISSED' GROUP BY " + COL_HIST_MED_NAME +
                " ORDER BY count DESC LIMIT 1", null);
        String result = null;
